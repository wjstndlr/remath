...
export default function PlaySessionPage() {
    ...
    async function handleMark(result: "mastered" | "review") {
        ...
        // 트랜지션 효과 발동 (좌/우 스와이프)
        setSlideDirection(result === "mastered" ? "right" : "left");
    ...
    return (
        <div className="flex h-screen flex-col overflow-hidden bg-slate-100">
            ...
            {/* 2-패널 본문 */}
            <div className={`flex flex-1 overflow-hidden flex-col lg:flex-row transition-all duration-400 ease-in-out ${slideDirection === 'left' ? '-translate-x-full opacity-0' : slideDirection === 'right' ? 'translate-x-full opacity-0' : 'translate-x-0 opacity-100'
                }`}>
                {/* 좌측: 문제 이미지 */}
                ...
                {/* 우측: 드로잉/해설/판정 패널 */}
                <div className="flex w-full lg:w-[clamp(400px,38vw,520px)] lg:border-l border-t lg:border-t-0 flex-col bg-white shadow-[-10px_0_30px_rgba(0,0,0,0.02)] z-20 border-slate-200 h-[50svh] lg:h-auto overflow-hidden" ref={panelRef}>
                    <div className="flex flex-col h-full animate-fade-in">
                        {/* 탭 헤더 */}
                        <div className="flex p-2 bg-slate-50 border-b border-slate-200 shrink-0">
                            {(["draw", "memo", "hint"] as const).map((tab) => (
                                ...
                                >
                                    {tab === "draw" ? "✏️ 내 풀이" : tab === "memo" ? "📝 메모" : "💡 해설"}
                                </button>
                            ))}
                        </div>
                                ...
                                <div className="h-full overflow-auto p-6 space-y-6">
                                    ...
                                    {problem.handwriting_url ? (
                                        <div className="space-y-3">
                                            ...
                                            <div className="bg-white p-2 rounded-2xl border border-slate-200 shadow-sm relative overflow-hidden">
                                                {/* eslint-disable-next-line @next/next/no-img-element */}
                                                <img src={problem.handwriting_url} alt="손글씨 해설" className="w-full h-auto rounded-xl" />
                                            </div>
                                        </div>
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            <div className="bg-white p-2 rounded-2xl border border-slate-200 shadow-sm relative overflow-hidden">
                                                {/* eslint-disable-next-line @next/next/no-img-element */}
                                                <img src={problem.solution_url} alt="원본 해설" className="w-full h-auto rounded-xl" />
                                            </div>
                                        </div>
                                    ) : (
                                        <div className="space-y-4">
                                            <div className="bg-indigo-50/50 rounded-2xl p-6 border border-indigo-100 border-dashed text-center">
                                                <div className="text-3xl mb-3">🤖</div>
                                                <h4 className="text-sm font-bold text-indigo-900 mb-1">AI 맞춤 해설 생성 가능</h4>
                                                <p className="text-[11px] text-indigo-600 font-medium">유료 버전에서는 이미지 분석을 통해<br />실시간 AI 해설을 받아볼 수 있습니다.</p>
                                                <button className="mt-4 px-4 py-2 bg-indigo-600 text-white text-[11px] font-bold rounded-lg shadow-lg shadow-indigo-200">AI 해설 체험하기 (PRO)</button>
                                            </div>
                                        </div>
                                    )}

                                    <div className="bg-amber-50/70 rounded-2xl p-5 border border-amber-100/50">
                                        <h4 className="text-[10px] font-black text-amber-500 uppercase tracking-widest mb-3">Saved Memo</h4>
                                    ...
                                    <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm mb-4">
                                        ...
                                        <div className="space-y-4">
                                            <div className="flex justify-between items-center text-sm">
                                                ...
                                                <span className="font-bold text-slate-800">{problem.subject} · {problem.unit_tags[0] || '기타'}</span>
                                            </div>
                                            <div className="flex justify-between items-center text-sm">
                        ...
                        {/* 하단 판정 액션바 */}
                        <div className="p-4 sm:p-6 bg-white border-t border-slate-100 shadow-[0_-4px_20px_rgba(0,0,0,0.02)] shrink-0">
                            <div className="text-center mb-3 text-[10px] font-black text-slate-400 uppercase tracking-wider">복습 결과 판정</div>
                            <div className="flex gap-2 sm:gap-3">
                                <button
                                    onClick={() => handleMark("review")}
                                    className="flex-1 py-4 sm:py-5 rounded-[1rem] border-2 border-slate-200 bg-white text-slate-600 font-bold hover:bg-rose-50 hover:text-rose-600 hover:border-rose-200 transition-all min-h-[56px] text-[13px] sm:text-sm active:scale-95 flex flex-col items-center gap-1 leading-none"
                                >
                                ...
                                <button
                                    ...
                                    className="flex-1 py-4 sm:py-5 rounded-[1rem] bg-slate-900 text-white font-bold hover:bg-slate-800 transition-all shadow-xl shadow-slate-900/20 hover:shadow-slate-900/30 min-h-[56px] text-[13px] sm:text-sm active:scale-95 flex flex-col items-center gap-1 leading-none"
                                >
                                    <span className="text-lg">✅</span>
                                    <span>완벽해요</span>
                                </button>
